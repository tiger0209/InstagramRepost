...
@interface SearchTagsViewController () <UITableViewDataSource, UITableViewDelegate>
...
//       return;
   ...
   if(self.tags.count == 0) {
    ...
    // Make request
    [[InstagramClient sharedClient] getTagsForSearching :self.searchKey
                                                      cursor:self.cursor
                                                  completion:^(BOOL success, NSArray *tags, NSString *cursor)
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // Text labels
    cell.textLabel.text       = [NSString stringWithFormat:@"#%@", user.tagName];
    cell.detailTextLabel.text = [NSString stringWithFormat:@"%@ posts", [self getCountWithComma:user.mediaCount]];
    
    // Profile picture
    [cell.profilePictureContainerView.imageView setImage:[UIImage imageNamed:@"TabBarUser"]];
    ...
    cell.profilePictureContainerView.actionBlock = ^(id sender){
        ...
        collectionView.m_tagName = tag.tagName;
...
-(NSString *)getCountWithComma:(NSString *)count
{
    int postCount = [count intValue];
    NSString *display = [NSNumberFormatter localizedStringFromNumber:@(postCount)
