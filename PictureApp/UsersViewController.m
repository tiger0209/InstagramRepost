...
- (void)loadUsers
{
    if(self.selection == SelectionFollowers || self.selection == SelectionFollowing) {
        ...
        // Make request
        [[InstagramClient sharedClient] getUsersForSelection:self.selection
    ...
    } else if(self.selection == SelectionMutualFriends
           || self.selection == SelectionFans
           || self.selection == SelectionNonFollowers
           || self.selection == SelectionUnfollowers
           || self.selection == SelectionNewFollowers
           || self.selection == SelectionUnfollowed) {
        ...
        // Load users
        [[DataManagement sharedClient] usersForSelection:self.selection completion:^(NSArray *users, Selection selection) {
            ...
            if(users.count > 0) {
                ...
                messageIconViewFrame.origin.y     = 0.0;
                messageIconViewFrame.size.height -= UITabBarHeight;
                
                NSString *text;
                
                if(self.selection == SelectionUnfollowers) {
                    text = @"No unfollowers have been tracked yet";
                } else if(self.selection == SelectionNewFollowers) {
                    text = @"No new followers have been tracked yet";
                } else if(self.selection == SelectionFans || self.selection == SelectionMutualFriends || self.selection == SelectionNonFollowers) {
                    text = @"No users found for this selection";
                } else if(self.selection == SelectionUnfollowed) {
                    text = @"No unfollowed users have been tracked yet";
                }
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    // Set the title
    if(self.selection == SelectionFollowers) {
        self.navigationController.navigationBar.topItem.title = NSLocalizedString(@"Followers", nil);
    } else if(self.selection == SelectionFollowing) {
        self.navigationController.navigationBar.topItem.title = NSLocalizedString(@"Following", nil);
    }
